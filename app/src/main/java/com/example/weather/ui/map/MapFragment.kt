...
class MapFragment :
    ...
    override fun initView() {
        ...
        binding.favoriteLocationButton.setOnClickListener {
        }
    ...
    override fun initData() {
        ...
        arguments?.let { args ->
            if (mSelectedLatitude == 0.0 && mSelectedLongitude == 0.0) {
                ...
                mSelectedLatitude = mWeather?.latitude!!
                mSelectedLongitude = mWeather?.longitude!!
            }
            val dateTime = mWeather?.weatherCurrent?.dateTime
            if (dateTime?.let { dateTime -> Utils.checkTimeInterval(dateTime) } == true && mIsNetworkEnable) {
                viewModel.getWeatherRemote(mSelectedLatitude, mSelectedLongitude)
            } else {
                val id = mWeather?.id
